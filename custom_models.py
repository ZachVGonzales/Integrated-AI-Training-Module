...
"""
" DESCRIPTION: these classifiers shall only require pure text input and shall make predictions
"              about the type of objective/question/etc.. is given in the text
"""
class VisionSequenceClassifiers(nn.Module):
  def __init__(self, secondary_types: dict[str:list], base_checkpoint: str = "bert-base-uncased", model_dir: str | None = None, device: torch.device | None = None) -> None:
    super(VisionSequenceClassifiers, self).__init__()
    self.secondary_types = secondary_types
    self.tokenizer = BertTokenizer.from_pretrained(base_checkpoint)
    self.device = device
    self.label_nums = {stype:len(secondary_types[stype]) for stype in self.secondary_types.keys()}
    self.classifiers = {}

    if model_dir is not None:
      for stype in self.secondary_types.keys():
        path = Path(model_dir, stype+"_classifier")
        if os.path.exists(str(path.resolve())):
          self.classifiers[stype] = BertForSequenceClassification.from_pretrained(str(path.resolve()), num_labels=self.label_nums[stype], local_files_only=True)
        else:
          self.classifiers[stype] = BertForSequenceClassification.from_pretrained(base_checkpoint, num_labels=self.label_nums[stype])
    else:
      for stype in self.secondary_types.keys():
        self.classifiers[stype] = BertForSequenceClassification.from_pretrained(base_checkpoint, num_labels=self.label_nums[stype])
  
  def forward(self, stype: str,
              input_ids: torch.Tensor | None = None, 
              attention_mask: torch.Tensor | None = None, 
              labels: torch.Tensor | None = None) -> tuple:
    
    classifier = self.classifiers[stype]
    try:
      if labels is None:
      ...
      print(f"exception {e}", file=sys.stderr)
      torch.set_printoptions(threshold=10000)
      print(input_ids.shape, input_ids, file=sys.stderr)
      print(attention_mask.shape, attention_mask, file=sys.stderr)
      torch.set_printoptions(threshold=100)
      return None, None

  def save(self, model_dir: str) -> None:
    for type in self.classifiers.keys():
      path = Path(model_dir, type+"_classifier")
      if not os.path.exists(str(path.resolve())):
        os.makedirs(str(path.resolve()))
      self.classifiers[type].save_pretrained(str(path.resolve()))